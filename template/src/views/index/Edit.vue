...
import { showMessage, mergeHasSource } from '@/utils';
...
const { route, router, store } = useCommon();
...
const getEditInfo = (GoodsId: number) => {
  ...
  apiGetGoodsDetail({ GoodsId }, () => {
    ...
    if (res.Code === 0) {
      const { CanSoldCount, RoundCanSoldCount } = res.Data;
      formData.data = mergeHasSource(formData.data, res.Data);
...
const setConfirmLoading = (type: boolean): void => {
  confirmLoading.value = type;
};
const onClosed = () => {
  router.push({
    path: '/'
